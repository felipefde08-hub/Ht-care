...
function ErrorComponent({ error, reset }: { error: Error; reset: () => void }) {
  ...
  const message = getFriendlyRouteError(error);
  useEffect(() => {
...
function getFriendlyRouteError(error: unknown) {
  ...
  if (normalized.includes("auth") || normalized.includes("jwt") || normalized.includes("session")) {
    return "Sua sessão não foi encontrada. Faça login novamente para continuar.";
  }
  if (normalized.includes("failed to fetch") || normalized.includes("network")) {
    return "Não foi possível conectar agora. Verifique sua internet e tente novamente.";
  }
  return "Tente atualizar a página. Se continuar, volte ao início e entre novamente.";
}
